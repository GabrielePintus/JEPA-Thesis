...
class VICRegJEPAEncoder(L.LightningModule):
    ...
    def shared_step(self, batch, batch_idx):
        ...
        vcreg_patch_curr = 15 * VCRegLoss.variance_loss(patch_curr_proj, 1.0) + VCRegLoss.covariance_loss(patch_curr_proj)
        vcreg_patch_next = 15 * VCRegLoss.variance_loss(patch_next_proj, 1.0) + VCRegLoss.covariance_loss(patch_next_proj)
        ...
        vcreg_cls_curr = 15 * VCRegLoss.variance_loss(cls_curr_proj, 1.0) + VCRegLoss.covariance_loss(cls_curr_proj)
        vcreg_cls_next = 15 * VCRegLoss.variance_loss(cls_next_proj, 1.0) + VCRegLoss.covariance_loss(cls_next_proj)
        ...
        vcreg_state_curr = 15 * VCRegLoss.variance_loss(z_state_curr_proj, 1.0) + VCRegLoss.covariance_loss(z_state_curr_proj)
        vcreg_state_next = 15 * VCRegLoss.variance_loss(z_state_next_proj, 1.0) + VCRegLoss.covariance_loss(z_state_next_proj)
